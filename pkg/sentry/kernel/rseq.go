...
func (t *Task) rseqAddrInterrupt() {
	...
	buf := t.CopyScratchBuffer(int(t.Arch().Width()))
	...
	// rseq is enabled. It must be as a fast as possible.
	//
	...
	end, ok := start.AddLength(cs.PostCommitOffset)
	if !ok {
	...
	critRange := usermem.AddrRange{Start: start, End: end}
