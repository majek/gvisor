...
func (fs *filesystem) newRegularFile(ctx context.Context, creds *auth.Credentials, mode linux.FileMode) *inode {
	file := &regularFile{
		memFile: kernel.KernelFromContext(ctx).MemoryFile(),
	}
...
func (rw *regularFileReadWriter) WriteFromBlocks(srcs safemem.BlockSeq) (uint64, error) {
	...
	for rw.off < end {
		...
		case gap.Ok():
			...
			seg, gap = rw.file.data.Insert(gap, gapMR, fr.Start), fileRangeGapIterator{}
		}
	}
exitLoop:
